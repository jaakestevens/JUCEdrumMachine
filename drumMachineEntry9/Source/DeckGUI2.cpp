...
//==============================================================================
DeckGUI2::DeckGUI2(DJAudioPlayer& dj, AudioFormatManager& formatManagerToUse, AudioThumbnailCache& cacheToUse, int bank)  :
...
void DeckGUI2::resized()
{
    ...
    volumeSlider.setBounds(getWidth() * .2, row * 2.5, getWidth() * .6, row * 1.5);
    speedSlider.setBounds(getWidth() * .2, row * 4.5, getWidth() * .7, row * 1.8);
    
    speedReset.setBounds(getWidth() * .8, row * 5, getWidth() * .2, row);
    waveformDisplay.setBounds(getWidth() * .1,row * 6.5, getWidth() * .8, row * 1.5);
    ...
    speedLabel.setBounds(getWidth() * .3, row * 4.15, getWidth() * .4, row);
...
void DeckGUI2::sliderValueChanged(Slider * slider)
{
    ...
    {
        float sliderSpeed = slider -> getValue();
        djAudioPlayer.setSpeed(sliderSpeed);
        std::cout<<"SpeedSlider"<<std::endl;
    }
}
